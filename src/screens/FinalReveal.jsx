...
const FinalReveal = () => {
  ...
  const { winner, playersWithScores } = calculateWinner();
  ...
  }, [gameState, showGender, babyGender]);
  ...
  const genderBgColor = babyGender === 'boy'
    ? 'from-blue-100 via-blue-50 to-white'
    : 'from-pink-100 via-pink-50 to-white';
  const genderEmoji = babyGender === 'boy' ? '👶💙' : '👶💗';
  const genderText = babyGender === 'boy' ? 'זה בן!' : 'זאת בת!';
            ...
            >
              ...
              <motion.h1
                ...
                className={`text-6xl font-black mb-4 ${
                  babyGender === 'boy' ? 'text-blue-600' : 'text-pink-600'
                }`}
              >
                {genderText}
              </motion.h1>
            ...
            <>
              ...
              >
                ...
                <div className="bg-white/20 rounded-2xl p-4">
                  <p className="text-xl">
                    פתח/ה {winner.correctCircles} עיגולים נכונים
                  </p>
                  <p className="text-lg opacity-90 mt-1">
                    מתוך {winner.keys} מפתחות!
                  </p>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="space-y-3">
                  {playersWithScores
                    .sort((a, b) => b.correctCircles - a.correctCircles)
                    .map((player, index) => {
                      const boyCount = player.circlesOpened.filter(c => c.gender === 'boy').length;
                      const girlCount = player.circlesOpened.filter(c => c.gender === 'girl').length;
                          ...
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <div className="text-center">
                              <div className="text-3xl font-black text-green-600">
                                {player.correctCircles}
                              </div>
                              <div className="text-xs text-gray-600">נכונים</div>
                            </div>
                          </div>
                          <div className="flex gap-3 justify-center">
                            <div className="bg-blue-100 rounded-2xl px-4 py-2 text-center flex-1">
                              <div className="text-2xl font-bold text-blue-600">{boyCount}</div>
                              <div className="text-xs text-blue-600">💙 בן</div>
                            </div>
                            <div className="bg-pink-100 rounded-2xl px-4 py-2 text-center flex-1">
                              <div className="text-2xl font-bold text-pink-600">{girlCount}</div>
